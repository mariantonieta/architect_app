...
export function useMaterialsForm(form: UseFormReturn<BudgetFormData>) {
    ...
    const validateMaterial = useCallback((m: Material) => ({
    ...
    const updateMaterial = useCallback((index: number, field: keyof Material, value: any) => {
        form.setValue(`materials.${index}.${field}`, value);
        const validation = validateMaterial({ ...materials[index], [field]: value });
        if (!validation.isValid) {
            if (validation.errors.item) toast.warning("El nombre del material es requerido");
            if (validation.errors.quantity) toast.warning("La cantidad debe ser mayor a 0");
        }
    ...
    return { materials, addMaterial, updateMaterial, deleteMaterial, move, validateMaterial };
}
